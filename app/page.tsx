  ...
  <header className="absolute top-0 left-0 w-full z-20 bg-transparent text-white">
    <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
      ...
      <div className="hidden md:flex space-x-6">
        ...
        <a href="/management" className="hover:text-blue-300">Management</a>
...
export default function HomePage() {
  return (
    <div className="bg-gray-100">
      <main>
        {/* Hero Grid Section */}
        <section className="h-screen grid grid-cols-2 grid-rows-2">
          {carsForSale.map((car) => (
            <div key={car.id} className="relative group overflow-hidden">
              ...
              <div className="absolute inset-0 bg-black bg-opacity-40 flex items-end p-6">
                <div className="transform transition-transform duration-500 ease-in-out group-hover:-translate-y-2">
                  ...
                  <p className="text-white text-lg drop-shadow-md">{car.year}</p>
                </div>
              </div>
            </div>
          ))}
